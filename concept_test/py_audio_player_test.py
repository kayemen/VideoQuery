...
def audiostream(queue, n_channels, sampling, n_bytes_per_sample):
    ...
    stream = p.open(format=p.get_format_from_width(n_bytes_per_sample),
                    channels=n_channels,
                    rate=sampling,
                    output=True)
...
audio_process = multiprocessing.Process(
    target=audiostream, args=(Q, channels, fs, bytes_per_sample))
audio_process.start()
# audio_process.join()
# stream = Q.get()
# audio_process.join()
...
# read data
audio = np.fromstring(wf.readframes(
    int(duration*fs*bytes_per_sample/channels)), dtype=dtype)
...
ax_spec_gram.specgram(audio[:, ch], Fs=fs, cmap='jet')
...
print("Setting up audioprint  process")
dt = .5
playhead = AudioSubsetter(audio, Q, channels, fs,
                          bytes_per_sample, chunk_dt=dt)
